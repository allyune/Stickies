...
router.get("/board/load/:uid", async (req, res) => {
  try {
    var stickies = await db.query(
      "SELECT id, color, content, position FROM stickies where board=$1 ORDER BY position",
      [req.params.uid]
    );
...
router.post("/stickie/add", async (req, res) => {
  try {
    let newStickie = await db.query(
      "INSERT INTO stickies (board) VALUES ($1) RETURNING id, color, content, position",
      [req.body.board]
    );
